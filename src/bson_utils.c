...
int make_atom_by_binary(ErlNifEnv *env, ERL_NIF_TERM *in, ERL_NIF_TERM *out) {
    ...
    if (!enif_make_existing_atom_len(env, (const char *)bin.data, bin.size, out,
                                     ERL_NIF_LATIN1)) {
        *out = *in;
    }
    return 1;
...
ERL_NIF_TERM split_by_char(ErlNifEnv *env, int argc,
    ...
    unsigned char *pos =
        (unsigned char *)memchr(bin.data + start, c, bin.size - start);
    if (pos == NULL) {
        return enif_make_badarg(env);
    }

    ERL_NIF_TERM begin =
        enif_make_sub_binary(env, argv[0], start, pos - bin.data - start);
    ERL_NIF_TERM rest = enif_make_sub_binary(env, argv[0], pos - bin.data + 1,
                                             bin.data + bin.size - pos - 1);
