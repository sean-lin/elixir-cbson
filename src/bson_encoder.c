...
inline static char check_ename(Encoder *e, unsigned char *bin, int32_t size) {
    return memchr(bin, 0x0, size) == NULL;
}

static Stack *enc_curr(Encoder *);

static ErlNifBinary *enc_alloc_bin(Encoder *e) {
    if (e->bin_top >= e->bin_size) {
        int new_sz = e->bin_size + BIN_ARRAY_SIZE;
        ErlNifBinary *tmp_ptr =
            (ErlNifBinary *)enif_alloc(new_sz * sizeof(ErlNifBinary));
...
inline static int enc_write_len(Encoder *e) {
    return e->i + e->bin_top ? (e->bin_top * BIN_BLOCK_SIZE) : 0;
}

inline static int enc_write_bin(Encoder *e, uint8_t *data, size_t len) {
...
static unsigned char *enc_skip_len(Encoder *e, int32_t len) {
    ErlNifBinary *bin = e->bin + e->bin_top - 1;
...
inline static void enc_write_uint8(Encoder *e, uint8_t d) {
    unsigned char *ptr = enc_skip_len(e, sizeof(uint8_t));
    *ptr = d;
}

inline static void enc_string(Encoder *e, unsigned char *data, int32_t size) {
    size += 1;
...
inline static int enc_atom(Encoder *e, ERL_NIF_TERM val) {
    char atom[512];
...
static int enc_ename(Encoder *e, ERL_NIF_TERM val) {
    char atom[512];
    ErlNifBinary bin;
...
inline static void enc_idx_ename(Encoder *e, int32_t val) {
    char str[32];
    int len = snprintf(str, 32, "%d", val);
    enc_write_bin(e, (unsigned char *)str, len + 1);
}

inline static int enc_objectid(Encoder *e, ERL_NIF_TERM doc) {
    ERL_NIF_TERM oid;
    ErlNifBinary bin;
...
inline static int enc_utc(Encoder *e, ERL_NIF_TERM doc) {
    ERL_NIF_TERM utc;
    ErlNifSInt64 ms;
...
inline static int enc_regex(Encoder *e, ERL_NIF_TERM doc) {
    ERL_NIF_TERM val;
    ErlNifBinary bin;
...
inline static int enc_timestamp(Encoder *e, ERL_NIF_TERM doc) {
    ERL_NIF_TERM ts;
    ErlNifSInt64 ms;
...
inline static int enc_bin(Encoder *e, ERL_NIF_TERM doc) {
    ERL_NIF_TERM val;
    ErlNifBinary bin;
    int32_t s;
    ...
    if (enif_get_map_value(e->env, doc, e->atoms->atom_bin_subtype, &val)) {
        if (enif_get_int(e->env, val, &s)) {
            if (enif_get_map_value(e->env, doc, e->atoms->atom_bin_value,
                                   &val)) {
                if (enif_inspect_binary(e->env, val, &bin)) {
...
inline static int enc_stack_is_list(Stack *st) {
    return st->status >= 0;
}

inline static Stack *enc_curr(Encoder *e) {
    return e->st_data + e->st_top - 1;
}

static void enc_push(Encoder *e, int32_t status) {
    Stack *st;
...
static Encoder *enc_new(ErlNifEnv *env) {
...
inline static ERL_NIF_TERM enc_error(Encoder *e, const char *err) {
    return make_error(e->atoms, e->env, err);
}

inline static ERL_NIF_TERM enc_obj_error(Encoder *e, const char *err,
                                         ERL_NIF_TERM obj) {
    return make_obj_error(e->atoms, e->env, err, obj);
}

inline static ERL_NIF_TERM make_last(ErlNifEnv *env, Encoder *e) {
    ErlNifBinary *bin = e->bin + e->bin_top - 1;
    if (e->i != bin->size) {
...
static ERL_NIF_TERM make_result(ErlNifEnv *env, Encoder *e) {
    ...
    for (int i = e->bin_top - 2; i >= 0; i--) {
        bin = e->bin + i;
        ret = enif_make_list_cell(env, enif_make_binary(env, bin), ret);
    }
    e->bin_top = 0;
    return ret;
}
...
ERL_NIF_TERM encode_init(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    if (enif_is_list(env, argv[0])) {
        ...
        enif_map_iterator_create(env, argv[0], &(enc_curr(e)->iter),
...
ERL_NIF_TERM encode_iter(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    next:
        ...
        that the map belongs to. if(should_yield(env, &start, enc_write_len(e),
        e->bytes_per_red)) { stack = enif_make_list_cell(env, curr, stack);
        ...
        */
        ...
        if (enc_stack_is_list(curr_stack)) {
            ...
            if (enif_is_list(env, curr)) {
                ...
                if (!enif_map_iterator_get_pair(env, &(curr_stack->iter), &key,
                                                &value)) {
            ...
            if (enif_is_atom(env, key) &&
                enif_compare(key, st->atom_struct) == 0) {
                goto next;
            }
        ...
        if (enif_is_binary(env, value)) {
            ...
            *ptr = BSON_DOUBLE;
            ...
            encode_map_doc:
                *ptr = BSON_DOCUMENT;
                enc_push(e, STACK_TYPE_DOC_MAP);
                enif_map_iterator_create(env, value, &(enc_curr(e)->iter),
                                         ERL_NIF_MAP_ITERATOR_FIRST);
