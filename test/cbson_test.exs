defmodule CBsonTest do
  ...
  test "encode/decode large" do
    t =
      Enum.reduce(1..500, %{}, fn x, acc ->
        Map.put(acc, to_string(x), :binary.copy("12345678910", x))
      end)
  ...
  test "decode cmd" do
    bin =
      <<92, 0, 0, 0, 16, 99, 111, 110, 110, 101, 99, 116, 105, 111, 110, 73, 100, 0, 26, 22, 0, 0,
        8, 117, 112, 100, 97, 116, 101, 100, 69, 120, 105, 115, 116, 105, 110, 103, 0, 0, 16, 110,
        0, 0, 0, 0, 0, 16, 115, 121, 110, 99, 77, 105, 108, 108, 105, 115, 0, 0, 0, 0, 0, 10, 119,
        114, 105, 116, 116, 101, 110, 84, 111, 0, 10, 101, 114, 114, 0, 1, 111, 107, 0, 0, 0, 0,
        0, 0, 0, 240, 63, 0>>
  ...
  test "save struct" do
    ...
    assert CBson.encode(%{key: t}) |> CBson.decode([:return_atom]) == %{
             key: :maps.remove(:__struct__, t)
           }
  end
  ...
  test "objectid" do
    assert "1234567890220200f3abcdef" ==
             Bson.ObjectId.from_string("1234567890220200f3abcdef") |> to_string()
