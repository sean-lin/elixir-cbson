        ...
        g: %Bson.Bin{
          ...
          bin:
            <<49, 0, 0, 0, 4, 66, 83, 79, 78, 0, 38, 0, 0, 0, 2, 48, 0, 8, 0, 0, 0, 97, 119, 101,
              115, 111, 109, 101, 0, 1, 49, 0, 51, 51, 51, 51, 51, 51, 20, 64, 16, 50, 0, 194, 7,
              0, 0, 0, 0>>
        },
        h: %Bson.Bin{
...
defmodule EncodeBench do
  ...
  bench "to_plain_json(elixir)", bson: Data.get_bson() do
    bson
    |> Enum.map(fn x ->
      x
      |> Bson.decode()
      |> Bson.JsonExt.plain_dump()
    end)
  end

  bench "to_plain_json(c)", bson: Data.get_bson() do
    bson |> Enum.map(fn x -> CBson.decode(x, [:return_json]) end)
  end
